...
| `main.py`, `main_niveau2.py`, `main_niveau3.py` | Command-line demo entry points, showcasing the pipeline on simulated cases with increasing feature depth |
| `app.py` | Desktop GUI (Tkinter) so non-technical hospital staff can run a diagnosis without writing code |

> Note: the `main_niveauX.py` / `report_niveauX.py` files represent the progressive iterations of the project (from a simple classifier to a full predictive/reporting system), kept for reference.
...
Run the command-line demo (generates synthetic X-ray cases with different artifacts and produces PDF reports):
...
> Output directories (`outputs/`, `historique/`) are created automatically at runtime, relative to the project folder.
